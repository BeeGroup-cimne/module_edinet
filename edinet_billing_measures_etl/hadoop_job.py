...
class Hadoop_ETL(MRJob):
    ...
    def mapper(self, _, doc):   #we don't have value -> input protocol pickleValue which means no key is read
        ...
        doc = self.datetime_to_timestamp(doc,'timestamp_start_bill')
        doc = self.datetime_to_timestamp(doc,'timestamp_end_bill')

        # ROW KEY DEFINITION
        row_key = self.build_row_key(doc)
        
        # Key - Value lists from the values dictionary
        doc_key = ['m:'+str(item[0]) for item in doc['values'].iteritems() if self.is_float(str(item[1]))]
        doc_val = [self.convert_units_to_kilo(doc['reading']['unit'], item) for item in doc['values'].itervalues() if self.is_float(str(item))]

        doc_key.extend(['m:v']) # column value, column calculated
        doc_val.extend([np.sum(doc_val)]) # Sum all instant period values, Consider a 0 because the measure is not treated with the cumulative to instant ETL
