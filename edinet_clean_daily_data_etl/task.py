        ...
        if data_type == "billing":
            mr_job = MRJob_clean_billing_data(
                args=['-r', 'hadoop', 'hdfs://' + input, '--file', f.name, '-c', 'module_edinet/edinet_clean_daily_data_etl/mrjob.conf',
                    ...
                    '--jobconf', 'mapred.job.name=edinet_clean_daily_data_etl_billing', '--jobconf', 'mapred.reduce.tasks={}'.format(self.num_reducers)])
        elif data_type == "metering":
                mr_job = MRJob_clean_metering_data(
                    args=['-r', 'hadoop', 'hdfs://' + input, '--file', f.name, '-c', 'module_edinet/edinet_clean_daily_data_etl/mrjob.conf',
                        '--output-dir', 'hdfs://' + output,
                        '--jobconf', 'mapred.job.name=edinet_clean_daily_data_etl_metering', '--jobconf', 'mapred.reduce.tasks={}'.format(self.num_reducers)])
        else:
                ...
                for companyId in data_companyId:
                    try:
                        ...
                        if table_name not in tables_list:
                            continue
                        hbase_table_name = "{}{}{}".format(self.config['hbase']['db'], self.config['hbase']['db_separator'],table_name)
