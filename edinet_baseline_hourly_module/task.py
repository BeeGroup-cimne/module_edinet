...
import sys
import pandas as pd
from module_edinet.edinet_baseline_hourly_module.align_job import MRJob_align
...
class BaselineModule(BeeModule2):
    ...
    """
    def __init__(self):
        super(BaselineModule, self).__init__("edinet_baseline_hourly_module")
    ...
    def launcher_hadoop_job(self, type, input, company=None, devices=None, stations=None, map_tasks=8, red_tasks=8):
        ...
        mr_job = MRJob_align(
            args=['-r', 'hadoop', 'hdfs://' + input, '--file', f.name, '-c', 'module_edinet/edinet_baseline_hourly_module/mrjob.conf',
                  '--jobconf', 'mapred.map.tasks=%s' % map_tasks, '--jobconf', 'mapred.reduce.tasks=%s' % red_tasks, '--jobconf', 'mapred.job.name=edinet_baseline_hourly_module'])
